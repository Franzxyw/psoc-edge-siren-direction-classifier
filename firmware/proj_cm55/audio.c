...
/* Stabilize the displayed result using a rolling majority vote. Predictions
 * below the confidence threshold are stored as invalid votes. */
#define PREDICTION_WINDOW_SIZE                  (5u)
#define PREDICTION_MIN_VOTES                    (3u)
#define INVALID_PREDICTION                      (-1)
...
/* Rolling prediction history used to suppress isolated classification jumps. */
static int16_t prediction_history[PREDICTION_WINDOW_SIZE] =
{
    INVALID_PREDICTION,
    INVALID_PREDICTION,
    INVALID_PREDICTION,
...
static uint32_t prediction_history_index;
...
static int16_t update_stable_prediction(int16_t prediction);
static bool is_unlabeled_output(const char *label);
static const char *physical_direction_label(const char *label);
...
/*******************************************************************************
* Function Name: update_stable_prediction
********************************************************************************
* Summary:
*  Adds a prediction to the rolling history and returns a label only when it
*  reaches the configured majority threshold.
*
* Parameters:
*  prediction: Model output label index or INVALID_PREDICTION.
*
* Return:
*  Stable label index or INVALID_PREDICTION when no label has enough votes.
*
*******************************************************************************/
static int16_t update_stable_prediction(int16_t prediction)
{
    uint32_t label_votes[IMAI_DATA_OUT_COUNT] = {0};
    int16_t stable_label = INVALID_PREDICTION;
    uint32_t highest_vote_count = 0;

    prediction_history[prediction_history_index] = prediction;
    prediction_history_index =
        (prediction_history_index + 1u) % PREDICTION_WINDOW_SIZE;

    for (uint32_t index = 0; index < PREDICTION_WINDOW_SIZE; index++)
    {
        int16_t label = prediction_history[index];
        if ((label >= 0) && (label < IMAI_DATA_OUT_COUNT))
        {
            label_votes[label]++;
        }
    }

    for (uint32_t label = 0; label < IMAI_DATA_OUT_COUNT; label++)
    {
        if ((label_votes[label] >= PREDICTION_MIN_VOTES) &&
            (label_votes[label] > highest_vote_count))
        {
            highest_vote_count = label_votes[label];
            stable_label = (int16_t)label;
        }
    }
...
static bool is_unlabeled_output(const char *label)
{
    return (0 == strcmp(label, "unlabeled")) ||
           (0 == strcmp(label, "{unlabeled}"));
}

static const char *physical_direction_label(const char *label)
{
    if (0 == strcmp(label, "East"))
    {
        return "West";
    }

    if (0 == strcmp(label, "West"))
    {
        return "East";
    }
...
cy_rslt_t pdm_data_process(void)
{
    ...
    for (uint32_t index = 0; index < FRAME_SIZE ; index++)
    {
        ...
        /* Preserve the channel order used by the original deployment code. */
            ...
            case IMAI_RET_SUCCESS:      /* We have data, display it */
            {
                for(int i = 0; i < IMAI_DATA_OUT_COUNT; i++)
                {
                    printf("label: %-11s: score: %.4f\r\n",
                           physical_direction_label(label_text[i]),
                           label_scores[i]);
                ...
                int16_t accepted_label = INVALID_PREDICTION;
                if ((max_score >= OUTPUT_THRESHOLD_SCORE) &&
                    !is_unlabeled_output(label_text[best_label]))
                {
                    accepted_label = best_label;
                    printf("\nRaw output: %-10s (%.4f)\r\n",
                           physical_direction_label(label_text[best_label]),
                           max_score);
                }
                else
                {
                    printf("\nRaw output: Unknown    (%.4f)\r\n", max_score);
                }

                stable_label = update_stable_prediction(accepted_label);
                if (stable_label != INVALID_PREDICTION)
                {
                    printf("Stable output: %-10s\r\n",
                           physical_direction_label(label_text[stable_label]));
                }
                else
                {
                    printf("Stable output: Unknown\r\n");
                }
